...
def crear_ventanas(precios: np.ndarray, k: int = 50):
    ...
    for t in range(T):
        ...
        # Resultado: (k, 2n) — exactamente la estructura del paper
        X[t] = np.column_stack([
...
# ============================================================
# MÓDULO 2: Arquitectura LSTM — f(θ|x_t) → w
# ============================================================
...
# ============================================================
# MÓDULO 3: Función objetivo — Sharpe Ratio como L_T
# ============================================================
...
def entrenar(modelo, X_train, r_train, X_val, r_val,
             epochs=100, lr=0.001, batch_size=64, verbose=True):
    """
    ...
    for epoch in range(epochs):
        ...
        train_sharpe = -epoch_loss / n_batches
...
def _run_k(precios, activos, k, hidden=64, epochs=100,
           batch_size=64, lr=0.001, val_split=0.1):
